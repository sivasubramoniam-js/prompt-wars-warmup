...
export default function UploadForm({ onSubmit, isProcessing }) {
  ...
  return (
    <section className="glass-panel p-6 sm:p-8 w-full max-w-2xl mx-auto backdrop-blur-3xl transition-all" aria-labelledby="form-heading">
      <h2 id="form-heading" className="text-2xl font-semibold mb-6 text-center text-slate-100">Emergency Feed</h2>
      
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label htmlFor="situation-description" className="block text-slate-300 text-sm font-medium mb-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <button 
              type="button"
              aria-label={image ? `Change uploaded image: ${image.name}` : "Upload Photo"}
              className={`border w-full border-slate-700/50 rounded-xl p-4 flex flex-col items-center justify-center text-center transition-all group focus:outline-none focus:ring-2 focus:ring-blue-400 ${image ? 'bg-blue-500/10 border-blue-500/30' : 'bg-slate-900/50 hover:bg-slate-800'}`} 
            ...
            <button 
              type="button"
              aria-label={isRecording ? "Stop recording audio" : audioBlob ? "Audio recorded successfully. Click to record again" : "Start recording audio"}
              className={`border w-full border-slate-700/50 rounded-xl p-4 flex flex-col items-center justify-center text-center transition-all group focus:outline-none focus:ring-2 focus:ring-blue-400 ${isRecording ? 'bg-red-500/10 border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.2)]' : audioBlob ? 'bg-green-500/10 border-green-500/30' : 'bg-slate-900/50 hover:bg-slate-800'}`}
        ...
        >
          {isProcessing ? (
            <><Loader2 className="animate-spin" aria-hidden="true" /> Analyzing Situation...</>
          ) : (
            <><Send size={20} aria-hidden="true" /> Request Emergency Assessment</>
          )}
        </button>
      </form>
    </section>
